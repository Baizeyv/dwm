...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
        ...
        if(ev->x < x){
            ...
            /* 2px right padding */
            if (ev->x > (x = selmon->ww - TEXTW(stext) + lrpad - 2 - getsystraywidth())){
                click = ClkStatusText;
                char *text = rawstext;
                int i = -1;
                char ch;
...
killunsel(const Arg *arg)
{
	...
	for (i = selmon->clients; i; i = i->next) {
		if (ISVISIBLE(i) && i != selmon->sel) {
			if (!sendevent(i, wmatom[WMDelete],0,0,0,0,0,0)) {
				XGrabServer(dpy);
				XSetErrorHandler(xerrordummy);
				XSetCloseDownMode(dpy, DestroyAll);
...
tile(Monitor *m)
{
	...
	for (i = 0, my = ty = m->gap->gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->rmaster ? m->wx + m->ww - mw : m->wx + m->gap->gappx, m->wy + my, mw - (2*c->bw) - m->gap->gappx, h - (2*c->bw), 0);
			my += HEIGHT(c) + m->gap->gappx;
            mfacts -= c->cfact;
		} else {
			h = (m->wh - ty) * (c->cfact / sfacts) - m->gap->gappx;
			resize(c, m->rmaster ? m->wx + m->gap->gappx : m->wx + mw + m->gap->gappx, m->wy + ty, m->ww - mw - (2*c->bw) - 2*m->gap->gappx, h - (2*c->bw), 0);
